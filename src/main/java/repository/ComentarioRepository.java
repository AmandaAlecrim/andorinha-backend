...
public class ComentarioRepository extends AbstractCrudRepository {
	...
	public void criarFiltro(StringBuilder sql, ComentarioSeletor seletor) {
		if (seletor.possuiFiltro()) {
			...
			if (seletor.getData() != null) {
				...
				sql.append("data_postagem = ? ");
			}
		}
	}

	public void adicionarParametro(PreparedStatement ps, ComentarioSeletor seletor) throws SQLException {
		int i = 1;
		...
		if (seletor.possuiFiltro()) {
			...
			if (seletor.getIdUsuario() != null) {
				ps.setInt(i++, seletor.getIdUsuario());
				System.out.println(i);
			}
			if (seletor.getIdTweet() != null) {
				ps.setInt(i++, seletor.getIdTweet());
			}
			if (seletor.getComentario() != null && !seletor.getComentario().trim().isEmpty()) {
				ps.setString(i++, String.format("%%%s%%", seletor.getComentario()));
			}
			if (seletor.getData() != null) {
				ps.setTimestamp(i++, new Timestamp(seletor.getData().getTimeInMillis()));
			}
		}
		System.out.println(i);
	}
	...
	public List<Comentario> pesquisar(ComentarioSeletor seletor) throws ErroAoConsultarBaseException {
		try (Connection c = super.ds.getConnection()) {
			...
			throw new ErroAoConsultarBaseException("Ocorreu um erro ao buscar os comentários", e);
		}
	}
